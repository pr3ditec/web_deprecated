                                        ...
                                        <div
                                            ...
                                            <template v-if="task.tags?.length">
                                                <template
                                                    ...
                                                    :key="i">
                                                    <div
                                                        class="btn px-2 py-1 flex btn-outline-primary">
                                                        <svg
                                                            width="24"
                                                            height="24"
                                                            viewBox="0 0 24 24"
...
const addEditTask = (projectId: any, task: any = null) => {
    ...
    if (task) {
        ...
        paramsTask.value.tags = paramsTask.value.tags
            ? paramsTask.value.tags.toString()
            : "";
    }
...
const saveTask = () => {
    ...
    const project = projectList.value.find(
        (d: any) => d.id === paramsTask.value.projectId,
    );
    if (paramsTask.value.id) {
        //update task
        const task = project.tasks.find(
            (d: any) => d.id === paramsTask.value.id,
        );
        task.title = paramsTask.value.title;
        task.description = paramsTask.value.description;
        task.tags =
            paramsTask.value.tags?.length > 0
                ? paramsTask.value.tags.split(",")
                : [];
    } else {
        //add task
        let maxid = 0;
...
const deleteTask = () => {
    let project = projectList.value.find(
        (d: any) => d.id === selectedTask.value.projectId,
    );
    project.tasks = project.tasks.filter(
        (d: any) => d.id != selectedTask.value.id,
    );
