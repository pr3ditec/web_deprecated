        ...
        <h1 class="text-3xl font-bold capitalize">{{ $t('join') }} {{ $t('clinic') }}</h1>
            ...
            <div class="w-full flex flex-col gap-0 p-6">
                ...
                <div class="flex flex-col w-full justify-center">
                    ...
                    <input class="form-input" type="text" v-mask="'###.###.###-##'" placeholder="Digite o cpf ........" v-on:keyup="$event => pesquisarClinica($event.target.value)"/>
                    <Transition>
                        <div class="w-full mt-2 bg-white shadow-[4px_6px_10px_-3px_#bfc9d4] rounded border border-[#e0e6ed] dark:border-[#1b2e4b] dark:bg-[#191e3a] dark:shadow-none" v-show="clinicas.length != 0">
                            <div class="p-5 flex items-center flex-col sm:flex-row">
                                ...
                                <h5 class="text-[#3b3f5c] text-[15px] font-semibold mb-2 dark:text-white-light">{{ clinicas.medico }}</h5>
            ...
            <div class="flex flex-col items-center font-semibold mt-2">
                <button class="btn btn-primary w-80 capitalize" @click="criarVinculo" :disabled="clinicas.length == 0">{{ $t('join') }}</button>
            </div>
    </div>
